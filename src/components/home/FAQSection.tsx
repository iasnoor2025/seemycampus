        ...
        >
          ...
          <div className="absolute top-1/2 -left-4 -right-4 -translate-y-1/2 flex justify-between z-20 pointer-events-none opacity-0 group-hover/carousel:opacity-100 transition-all duration-500">
            <button
              ...
              className="w-12 h-12 rounded-2xl bg-white/90 backdrop-blur-md shadow-xl flex items-center justify-center text-slate-800 hover:bg-blue-600 hover:text-white transition-all pointer-events-auto border border-white/50"
              aria-label="Previous"
            >
          ...
          <div
            ...
            className="overflow-x-auto flex snap-x snap-mandatory scrollbar-hide py-8"
          >
              ...
              Array.from({ length: maxSlides }).map((_, slideIndex) => (
                <div key={`${blockIndex}-${slideIndex}`} className="min-w-full snap-start snap-always flex gap-6 px-4">
                  {faqs
                    ...
                    .map((faq, index) => {
                      const actualIndex = slideIndex * itemsPerView + index
                      return (
                        <div
                          key={`${blockIndex}-${actualIndex}`}
                          className="flex-1 bg-white rounded-[2.5rem] p-8 shadow-[0_10px_40px_rgba(0,0,0,0.03)] hover:shadow-[0_25px_60px_rgba(37,99,235,0.08)] transition-all duration-700 transform hover:-translate-y-3 border border-slate-100/50 group relative overflow-hidden flex flex-col min-h-[300px]"
                        >
                          {/* Decorative bubble */}
                          <div className={`absolute -top-12 -right-12 w-40 h-40 bg-gradient-to-br ${gradients[actualIndex % gradients.length]} opacity-0 group-hover:opacity-10 rounded-full transition-all duration-700 blur-3xl`}></div>

                          <div className="flex items-start gap-5 mb-6 relative z-10">
                            <div className={`shrink-0 w-12 h-12 rounded-2xl bg-gradient-to-br ${gradients[actualIndex % gradients.length]} text-white flex items-center justify-center font-black text-lg shadow-lg group-hover:scale-110 group-hover:rotate-6 transition-all duration-500`}>
                              {(actualIndex % faqs.length) + 1}
                            </div>
                            <h3 className="text-xl font-black text-slate-900 leading-tight group-hover:text-blue-600 transition-colors duration-300 tracking-tight">
                              {faq.question}
                            </h3>
                          </div>

                          <p className="text-slate-600 leading-relaxed font-bold text-sm relative z-10">
                            {faq.answer}
                          </p>
                        </div>
                      )
                    })}
                  {/* Fill empty spaces if necessary */}
                  {slideIndex === maxSlides - 1 &&
                    faqs.slice(slideIndex * itemsPerView, (slideIndex + 1) * itemsPerView).length < itemsPerView &&
          ...
          <div className="flex justify-center gap-2 mt-4">
            {Array.from({ length: maxSlides }).map((_, index) => (
              <button
                ...
                className={`rounded-full transition-all duration-500 ${index === currentSlide ? "w-8 h-1.5 bg-blue-600" : "w-1.5 h-1.5 bg-blue-200 hover:bg-blue-300"}`}
